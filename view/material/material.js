...
let intl = {};
let selectedLibrary = {};
let snippetsInstallLoading = false;
...
window.addEventListener('message', (event) => {
  ...
  if (message.snippets?.status === 'finish') {
    snippetsInstallLoading = false;
  }
});
...
function blockListRender(blocks) {
  ...
  function renderPageList() {
    ...
    // 安装全部代码片段
    $('#getSnippets').click(function () {
      if (!snippetsInstallLoading) {
        vscode.postMessage({ getSnippets: blocks });
        snippetsInstallLoading = true;
      }
    });
